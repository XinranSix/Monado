...
namespace Monado {
    ...
    OpenGLFramebuffer::OpenGLFramebuffer(const FramebufferSpecification &spec) : Framebuffer(spec.width, spec.height) {
        ...
        if (!m_EnableMSAA) {
            for (unsigned int i = 0; i < spec.colorAttachmentCnt; i++) {
                ...
                // TODO: 这样写死并不合适
                if (i == 0)
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, spec.width, spec.height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                 NULL);
                else
                    // R32I应该是代表32位interger, 意思是这32位都只存一个integer
                    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32I, spec.width, spec.height, 0, GL_RED_INTEGER,
            ...
            for (unsigned int i = 0; i < spec.depthAttachmentCnt; i++) {
                ...
                glRenderbufferStorage(
                    GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, spec.width,
                    spec.height); // use a single renderbuffer object for both a depth AND stencil buffer.
            ...
            for (size_t i = 0; i < spec.depthAttachmentCnt; i++) {
                ...
                glRenderbufferStorageMultisample(
                    GL_RENDERBUFFER, 4, GL_DEPTH24_STENCIL8, spec.width,
                    spec.height); // use a single renderbuffer object for both a depth AND stencil buffer.
                glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                          rbo); // now actually attach it
                m_RboAttachmentIndices.push_back(rbo);
        ...
        MONADO_CORE_ASSERT((bool)(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE),
    ...
    GLuint OpenGLFramebuffer::GetColorAttachmentId(uint32_t id) { return m_ColorAttachmentTexIndices[id]; }
    ...
    uint32_t OpenGLFramebuffer::GetFramebufferId() { return m_FramebufferId; }
    ...
    void OpenGLFramebuffer::ResolveMSAATexture(uint32_t width, uint32_t height) {
        MONADO_ASSERT(m_EnableMSAA, "Framebuffer must enable MSAA when resolve Textures!");
    ...
    OpenGLShader *OpenGLFramebuffer::GetOpenGLShader() { return (OpenGLShader *)(&*GetShader()); }
} // namespace Monado
