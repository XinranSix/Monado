...
#define MND_ADD_INTERNAL_CALL(Name) mono_add_internal_call("Monado.InternalCalls::" #Name, Name)
    ...
    static MonoString *TextComponent_GetText(UUID entityID) {
        ...
        Entity entity = scene->GetEntityByUUID(entityID);
        MND_CORE_ASSERT(entity);
        MND_CORE_ASSERT(entity.HasComponent<TextComponent>());
    ...
    static void RegisterComponent() {
        (
            []() {
                ...
                std::string managedTypename = fmt::format("Monado.{}", structName);

                MonoType *managedType =
                    mono_reflection_type_from_name(managedTypename.data(), ScriptEngine::GetCoreAssemblyImage());
                if (!managedType) {
                    MND_CORE_ERROR("Could not find component type {}", managedTypename);
                    return;
                }
    ...
    void ScriptGlue::RegisterFunctions() {
        ...
        MND_ADD_INTERNAL_CALL(Rigidbody2DComponent_GetLinearVelocity);
        MND_ADD_INTERNAL_CALL(Rigidbody2DComponent_GetType);
        MND_ADD_INTERNAL_CALL(Rigidbody2DComponent_SetType);

        MND_ADD_INTERNAL_CALL(TextComponent_GetText);
        MND_ADD_INTERNAL_CALL(TextComponent_SetText);
        MND_ADD_INTERNAL_CALL(TextComponent_GetColor);
        MND_ADD_INTERNAL_CALL(TextComponent_SetColor);
        MND_ADD_INTERNAL_CALL(TextComponent_GetKerning);
        MND_ADD_INTERNAL_CALL(TextComponent_SetKerning);
        MND_ADD_INTERNAL_CALL(TextComponent_GetLineSpacing);
        MND_ADD_INTERNAL_CALL(TextComponent_SetLineSpacing);
