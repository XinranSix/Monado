...
namespace Monado {
        ...
        if (Input::IsMouseButtonPressed(1)) {
            if (m_Camera.IsOrthographicCamera()) {
                ...
                if (Input::IsKeyPressed(MND_KEY_A))
                    ...
                    m_CameraPosition -= m_Camera.GetLocalRight() * (float)ts;
                else if (Input::IsKeyPressed(MND_KEY_W))
    ...
    void EditorCameraController::OnEvent(Event &e) {
        if (e.GetEventType() == Monado::EventType::MouseScrolled) {
            MouseScrolledEvent *ep = dynamic_cast<Monado::MouseScrolledEvent *>(&e);
            OnZoomCamera(ep->GetYOffset());
        }

        if (e.GetEventType() == Monado::EventType::MouseButtonReleased) {
            MouseButtonReleasedEvent *ep = dynamic_cast<Monado::MouseButtonReleasedEvent *>(&e);
        ...
        if (m_Camera.IsProjectiveCamera()) {
            if (Input::IsMouseButtonPressed(1)) {
                if (e.GetEventType() == Monado::EventType::MouseMoved) {
                    MouseMovedEvent *ep = dynamic_cast<Monado::MouseMovedEvent *>(&e);
    ...
    void EditorCameraController::OnZoomCamera(float scrollOffset) {
        if (m_Camera.IsOrthographicCamera()) {
            ...
            m_Camera.SetProjectionMatrixOrtho(-m_AspectRadio * m_ZoomLevel, m_AspectRadio * m_ZoomLevel, -m_ZoomLevel,
                                              m_ZoomLevel);
        } else if (m_Camera.IsProjectiveCamera()) {
