...
namespace Monado {
    EditorCamera::EditorCamera(ProjectionType type, float leftOrFov, float rightOrAspect, float bottomOrZNear,
                               float topOrZFar) {
        // FIXME: 喵~~哉
    ...
    void EditorCamera::OnResize(uint32_t width, uint32_t height) {
        ...
        if (m_Type == ProjectionType::Orthographic) {
            ...
            // 而是会随着鼠标滚动改变zoom值而变化
            SetProjectionMatrixOrtho(-m_AspectRatio * m_ZoomLevel, m_AspectRatio * m_ZoomLevel, -m_ZoomLevel,
                                     m_ZoomLevel);
    ...
    void EditorCamera::SetProjectionMatrixOrtho(float left, float right, float bottom, float top) {
        MONADO_ASSERT(bool(m_Type == ProjectionType::Orthographic), "The camera must be orthographic!");
    ...
    void EditorCamera::SetProjectionMatrixPersp(float fov, float aspect, float zNear, float zFar) {
        MONADO_ASSERT(bool(m_Type == ProjectionType::Perspective), "The camera must be perspective!")
