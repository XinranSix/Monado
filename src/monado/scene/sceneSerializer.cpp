...
namespace YAML {
    ...
    template <>
    struct convert<Monado::UUID> {
        static Node encode(const Monado::UUID &uuid) {
            Node node;
            node.push_back((uint64_t)uuid);
            return node;
        }

        static bool decode(const Node &node, Monado::UUID &uuid) {
...
#define READ_SCRIPT_FIELD(FieldType, Type)                                                                             \
    ...
    bool SceneSerializer::Deserialize(const std::string &filepath) {
        ...
        MND_CORE_TRACE("Deserializing scene '{0}'", sceneName);
                ...
                if (scriptComponent) {
                    ...
                    if (scriptFields) {
                        ...
                        for (auto scriptField : scriptFields) {
                            ...
                            // TODO(Yan): turn this assert into Monadonut log warning
                            MND_CORE_ASSERT(fields.find(name) != fields.end());
