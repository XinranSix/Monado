...
namespace Monado {
    ...
    std::shared_ptr<Shader> ShaderLibrary::Get(const std::string &shaderName) {
        MONADO_CORE_ASSERT(Contains(shaderName), "Can't find shader:{0}", shaderName);

        return m_Map[shaderName];
    }

    bool ShaderLibrary::Contains(const std::string &shaderName) { return m_Map.find(shaderName) != m_Map.end(); }
} // namespace Monado
