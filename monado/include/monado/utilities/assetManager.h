...
namespace Monado {
    ...
    enum class AssetType { Scene, Mesh, Image, Audio, Script, Other };
    ...
    struct DirectoryInfo {
        ...
        DirectoryInfo(const std::string &filename, const std::string &fileType, const std::string &absolutePath,
                      bool isFile)
            : Filename(filename), FileType(fileType), AbsolutePath(absolutePath), IsFile(isFile) {}

        DirectoryInfo(const DirectoryInfo &other)
            : Filename(other.Filename), FileType(other.FileType), AbsolutePath(other.AbsolutePath),
              IsFile(other.IsFile) {}

        DirectoryInfo(DirectoryInfo &&other)
            : Filename(std::move(other.Filename)), FileType(std::move(other.FileType)),
              AbsolutePath(std::move(other.AbsolutePath)), IsFile(std::move(other.IsFile)) {}
