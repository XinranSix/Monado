        ...
        if (entity.HasComponent<ScriptComponent>()) {
            ...
            if (moduleFieldMap.find(moduleName) != moduleFieldMap.end()) {
                ...
                for (const auto &[name, field] : fields) {
                    ...
                    switch (field.Type) {
                    case FieldType::Int: out << field.GetStoredValue<int>(); break;
                    case FieldType::UnsignedInt: out << field.GetStoredValue<uint32_t>(); break;
                    case FieldType::Float: out << field.GetStoredValue<float>(); break;
                    case FieldType::Vec2: out << field.GetStoredValue<glm::vec2>(); break;
                    case FieldType::Vec3: out << field.GetStoredValue<glm::vec3>(); break;
                    case FieldType::Vec4: out << field.GetStoredValue<glm::vec4>(); break;
                    }
    ...
    bool SceneSerializer::Deserialize(const std::string &filepath) {
        ...
        if (entities) {
            for (auto entity : entities) {
                ...
                MND_CORE_INFO("Deserialized entity with ID = {0}, name = {1}", uuid, name);
                ...
                if (scriptComponent) {
                    ...
                    MND_CORE_INFO("  Script Module: {0}", moduleName);
                        ...
                        if (storedFields) {
                            for (auto field : storedFields) {
                                ...
                                case FieldType::String: {
                                    MND_CORE_ASSERT(false, "Unimplemented");
                                    break;
                                }
                                case FieldType::Vec2: {
                ...
                if (meshComponent) {
                    ...
                    if (meshComponent["AssetPath"])
                        assetHandle =
                            AssetManager::GetAssetHandleFromFilePath(meshComponent["AssetPath"].as<std::string>());
                    else
                    ...
                    if (AssetManager::IsAssetHandleValid(assetHandle)) {
                        ...
                        std::string filepath =
                            meshComponent["AssetPath"] ? meshComponent["AssetPath"].as<std::string>() : "";
                        if (filepath.empty())
                            MND_CORE_ERROR("Tried to load non-existent mesh in Entity: {0}",
                                           deserializedEntity.GetUUID());
                        else
                            MND_CORE_ERROR("Tried to load invalid mesh '{0}' in Entity {1}", filepath,
                                           deserializedEntity.GetUUID());
                    }
                }
                ...
                if (skyLightComponent) {
                    ...
                    if (skyLightComponent["EnvironmentAssetPath"])
                        assetHandle = AssetManager::GetAssetHandleFromFilePath(
                            skyLightComponent["EnvironmentAssetPath"].as<std::string>());
                    else
                    ...
                    if (AssetManager::IsAssetHandleValid(assetHandle)) {
                        ...
                        std::string filepath = meshComponent["EnvironmentAssetPath"]
                                                   ...
                                                   : "";
                        if (filepath.empty())
                            MND_CORE_ERROR("Tried to load non-existent environment map in Entity: {0}",
                                           deserializedEntity.GetUUID());
                        else
                            MND_CORE_ERROR("Tried to load invalid environment map '{0}' in Entity {1}", filepath,
                                           deserializedEntity.GetUUID());
                    }
                ...
                if (boxCollider2DComponent) {
                    ...
                    component.Density =
                        boxCollider2DComponent["Density"] ? boxCollider2DComponent["Density"].as<float>() : 1.0f;
                    component.Friction =
                        boxCollider2DComponent["Friction"] ? boxCollider2DComponent["Friction"].as<float>() : 1.0f;
                }
                ...
                if (circleCollider2DComponent) {
                    ...
                    component.Density =
                        circleCollider2DComponent["Density"] ? circleCollider2DComponent["Density"].as<float>() : 1.0f;
                ...
                if (boxColliderComponent) {
                    ...
                    if (material) {
                        if (AssetManager::IsAssetHandleValid(material.as<uint64_t>())) {
                            ...
                            MND_CORE_ERROR("Tried to load invalid Physics Material in Entity {0}",
                                           deserializedEntity.GetUUID());
                        }
                    }
                ...
                if (meshColliderComponent) {
                    ...
                    if (overrideMesh) {
                        AssetHandle assetHandle =
                            meshColliderComponent["AssetID"] ? meshColliderComponent["AssetID"].as<uint64_t>() : 0;
