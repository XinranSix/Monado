...
namespace Monado {
    ...
    Mesh::Mesh(const std::string &filename) : m_FilePath(filename) {
        ...
        m_Material.reset(new Monado::Material(m_MeshShader));
    ...
    void Mesh::TraverseNodes(aiNode *node, int level) {
        ...
        for (int i = 0; i < level; i++)
            levelText += "-";
        MND_CORE_TRACE("{0}Node name: {1}", levelText, std::string(node->mName.data));
    ...
    void Mesh::Render(TimeStep ts, const glm::mat4 &transform, Ref<MaterialInstance> materialInstance) {
        ...
        MND_RENDER_S2(transform, materialOverride, {
            for (Submesh &submesh : self->m_Submeshes) {
                if (self->m_IsAnimated) {
                    ...
                    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(AnimatedVertex),
                                          (const void *)offsetof(AnimatedVertex, Position));

                    glEnableVertexAttribArray(1);
                    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(AnimatedVertex),
                                          (const void *)offsetof(AnimatedVertex, Normal));

                    glEnableVertexAttribArray(2);
                    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(AnimatedVertex),
                                          (const void *)offsetof(AnimatedVertex, Tangent));

                    glEnableVertexAttribArray(3);
                    glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(AnimatedVertex),
                                          (const void *)offsetof(AnimatedVertex, Binormal));

                    glEnableVertexAttribArray(4);
                    glVertexAttribPointer(4, 2, GL_FLOAT, GL_FALSE, sizeof(AnimatedVertex),
                                          (const void *)offsetof(AnimatedVertex, Texcoord));

                    glEnableVertexAttribArray(5);
                    glVertexAttribIPointer(5, 4, GL_INT, sizeof(AnimatedVertex),
                                           (const void *)offsetof(AnimatedVertex, IDs));

                    glEnableVertexAttribArray(6);
                    glVertexAttribPointer(6, 4, GL_FLOAT, GL_FALSE, sizeof(AnimatedVertex),
                                          (const void *)offsetof(AnimatedVertex, Weights));
                    ...
                    glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
                                          (const void *)offsetof(Vertex, Binormal));
