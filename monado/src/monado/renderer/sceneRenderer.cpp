...
namespace Monado {
    ...
    void SceneRenderer::Shutdown() { delete s_Data; }
    ...
    static void CalculateCascades(CascadeData *cascades, const SceneRendererCamera &sceneCamera,
    ...
    void SceneRenderer::BeginScene(const Scene *scene, const SceneRendererCamera &camera) {
        ...
        if (s_Data->NeedsResize) {
            s_Data->GeometryPipeline->GetSpecification().RenderPass->GetSpecification().TargetFramebuffer->Resize(
                s_Data->ViewportWidth, s_Data->ViewportHeight);
            s_Data->CompositePipeline->GetSpecification().RenderPass->GetSpecification().TargetFramebuffer->Resize(
                s_Data->ViewportWidth, s_Data->ViewportHeight);
            s_Data->NeedsResize = false;
        }

        Renderer::SetSceneEnvironment(s_Data->SceneData.SceneEnvironment, s_Data->ShadowPassPipeline->GetSpecification()
                                                                              .RenderPass->GetSpecification()
                                                                              .TargetFramebuffer->GetDepthImage());
    ...
    void SceneRenderer::CompositePass() {
        ...
        auto framebuffer =
            s_Data->GeometryPipeline->GetSpecification().RenderPass->GetSpecification().TargetFramebuffer;
    ...
    Ref<Image2D> SceneRenderer::GetFinalPassImage() {
        return s_Data->CompositePipeline->GetSpecification()
            .RenderPass->GetSpecification()
            .TargetFramebuffer->GetImage();
    }
