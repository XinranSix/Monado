...
namespace Monado {
    ...
    void SceneRenderer::Init() {
        ...
        geoFramebufferSpec.Attachments = { FramebufferTextureFormat::RGBA16F, FramebufferTextureFormat::RGBA16F,
    ...
    void SceneRenderer::GeometryPass() {
        ...
        for (auto &dc : s_Data.DrawList) {
            ...
            if (rd) {
                ...
                auto tex =
                    s_Data.ShadowMapRenderPass[0]->GetSpecification().TargetFramebuffer->GetDepthAttachmentRendererID();
                auto tex1 =
                    s_Data.ShadowMapRenderPass[1]->GetSpecification().TargetFramebuffer->GetDepthAttachmentRendererID();
                auto tex2 =
                    s_Data.ShadowMapRenderPass[2]->GetSpecification().TargetFramebuffer->GetDepthAttachmentRendererID();
                auto tex3 =
                    s_Data.ShadowMapRenderPass[3]->GetSpecification().TargetFramebuffer->GetDepthAttachmentRendererID();
        ...
        if (outline) {
            ...
            for (auto &dc : s_Data.SelectedMeshDrawList) {
                Renderer::SubmitMesh(dc.Mesh, dc.Transform,
                                     dc.Mesh->IsAnimated() ? s_Data.OutlineAnimMaterial : s_Data.OutlineMaterial);
            }
    ...
    void SceneRenderer::CompositePass() {
        ...
        s_Data.CompositeShader->SetFloat2("u_ViewportSize",
    ...
    void SceneRenderer::BloomBlurPass() {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            if (i > 0) {
                ...
                Renderer::Submit([id]() { glBindTextureUnit(0, id); });
            }
            Renderer::SubmitFullscreenQuad(nullptr);
            Renderer::EndRenderPass();
        }
    ...
    static void CalculateCascades(CascadeData *cascades, const glm::vec3 &lightDirection) {
        ...
        for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
            ...
            glm::mat4 lightViewMatrix =
                glm::lookAt(frustumCenter - lightDir * -minExtents.z, frustumCenter, glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    void SceneRenderer::ShadowMapPass() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            static glm::mat4 scaleBiasMatrix =
                glm::scale(glm::mat4(1.0f), { 0.5f, 0.5f, 0.5f }) * glm::translate(glm::mat4(1.0f), { 1, 1, 1 });
    ...
    void SceneRenderer::SetFocusPoint(const glm::vec2 &point) { s_Data.FocusPoint = point; }
