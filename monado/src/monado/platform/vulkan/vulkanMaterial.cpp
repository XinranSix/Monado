...
namespace Monado {
    ...
    const ShaderUniform *VulkanMaterial::FindUniformDeclaration(const std::string &name) {
        ...
        MND_CORE_ASSERT(shaderBuffers.size() <= 1, "We currently only support ONE material buffer!");
    ...
    void VulkanMaterial::SetVulkanDescriptor(const std::string &name, const Ref<Texture2D> &texture) {
        ...
        MND_CORE_ASSERT(wds);
        m_ResidentDescriptors.push_back(std::make_shared<PendingDescriptor>(
    ...
    void VulkanMaterial::Set(const std::string &name, const Ref<Image2D> &image) { SetVulkanDescriptor(name, image); }
    ...
    Ref<Texture2D> VulkanMaterial::GetTexture2D(const std::string &name) { return GetResource<Texture2D>(name); }

    Ref<TextureCube> VulkanMaterial::TryGetTextureCube(const std::string &name) {
        return TryGetResource<TextureCube>(name);
    }

    Ref<Texture2D> VulkanMaterial::TryGetTexture2D(const std::string &name) { return TryGetResource<Texture2D>(name); }

    Ref<TextureCube> VulkanMaterial::GetTextureCube(const std::string &name) { return GetResource<TextureCube>(name); }
    ...
    void VulkanMaterial::UpdateForRendering() {
        ...
        Renderer::Submit([instance, writeDescriptors, residentDescriptors, pendingDescriptors]() mutable {
            ...
            for (auto &descriptor : residentDescriptors) {
                if (descriptor->Type == PendingDescriptorType::Image2D) {
                    ...
                    if (descriptor->WDS.pImageInfo &&
                        ...
                        MND_CORE_WARN("Found out of date Image2D descriptor ({0} vs. {1})",
                                      (void *)image->GetImageInfo().ImageView,
            ...
            if (writeDescriptors.size()) {
                ...
                MND_CORE_WARN("Updating {0} descriptor sets", writeDescriptors.size());
        ...
        Renderer::Submit([instance]() mutable {
            ...
            if (instance->m_WriteDescriptors.size()) {
                ...
                MND_CORE_WARN("VulkanMaterial - Updating {0} descriptor sets", instance->m_WriteDescriptors.size());
                vkUpdateDescriptorSets(vulkanDevice, instance->m_WriteDescriptors.size(),
                                       instance->m_WriteDescriptors.data(), 0, nullptr);
            }
