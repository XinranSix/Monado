...
#include "monado/platform/vulkan/vulkanSwapChain.h"
...
namespace Monado {
    ...
    VulkanTexture2D::VulkanTexture2D(const std::string &path, bool srgb) : m_Path(path) {
        ...
        MND_CORE_ASSERT(m_ImageData.Data, "Failed to load image!");
        m_Width = width;
        m_Height = height;

        MND_CORE_ASSERT(m_Format != ImageFormat::None);
    ...
    VulkanTexture2D::VulkanTexture2D(ImageFormat format, uint32_t width, uint32_t height, const void *data,
        ...
        MND_CORE_ASSERT(format == ImageFormat::RGBA);
    ...
    VulkanTexture2D::~VulkanTexture2D() {
        ...
        Renderer::Submit([image]() mutable { image->Release(); });
    }
    ...
    void VulkanTexture2D::Invalidate() {
        ...
        // The sub resource range describes the regions of the image that will be transitioned using the memory barriers
        // below
        VkImageSubresourceRange subresourceRange = {};
        ...
        vkCmdCopyBufferToImage(copyCmd, stagingBuffer, info.Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                               &bufferCopyRegion);
        ...
        Utils::InsertImageMemoryBarrier(copyCmd, info.Image, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT,
    ...
    bool VulkanTexture2D::Loaded() const { return true; }

    const std::string &VulkanTexture2D::GetPath() const { return m_Path; }

    uint32_t VulkanTexture2D::GetMipLevelCount() const { return Utils::CalculateMipCount(m_Width, m_Height); }
    ...
    void VulkanTexture2D::GenerateMips() {
        ...
        for (int32_t i = 1; i < mipLevels; i++) {
            ...
            Utils::InsertImageMemoryBarrier(blitCmd, info.Image, 0, VK_ACCESS_TRANSFER_WRITE_BIT,
            ...
            Utils::InsertImageMemoryBarrier(blitCmd, info.Image, VK_ACCESS_TRANSFER_WRITE_BIT,
        ...
        Utils::InsertImageMemoryBarrier(blitCmd, info.Image, VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_SHADER_READ_BIT,
    ...
    void VulkanTextureCube::Invalidate() {
        ...
        if (m_LocalStorage) {
            ...
            uint8_t *destData;
            VK_CHECK_RESULT(
                vkMapMemory(vulkanDevice, stagingMemory, 0, memoryRequirements.size, 0, (void **)&destData));
            ...
            Utils::InsertImageMemoryBarrier(copyCmd, m_Image, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT,
                                            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                            VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                            subresourceRange);
        ...
        SetImageLayout(layoutCmd, m_Image, VK_IMAGE_LAYOUT_UNDEFINED, m_DescriptorImageInfo.imageLayout,
                       subresourceRange);
    ...
    uint32_t VulkanTextureCube::GetMipLevelCount() const { return Utils::CalculateMipCount(m_Width, m_Height); }
    ...
    void VulkanTextureCube::GenerateMips(bool readonly) {
        ...
        for (uint32_t face = 0; face < 6; face++) {
            ...
            // Prepare current mip level as image blit destination
            Utils::InsertImageMemoryBarrier(blitCmd, m_Image, 0, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                            VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                            VK_PIPELINE_STAGE_TRANSFER_BIT, mipSubRange);
        }
        ...
        for (int32_t i = 1; i < mipLevels; i++) {
            for (uint32_t face = 0; face < 6; face++) {
                ...
                // Prepare current mip level as image blit destination
                Utils::InsertImageMemoryBarrier(blitCmd, m_Image, 0, VK_ACCESS_TRANSFER_WRITE_BIT,
                                                VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                                VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                                mipSubRange);
                ...
                // Prepare current mip level as image blit source for next level
                Utils::InsertImageMemoryBarrier(blitCmd, m_Image, VK_ACCESS_TRANSFER_WRITE_BIT,
                                                VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                                VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                                VK_PIPELINE_STAGE_TRANSFER_BIT, mipSubRange);
            }
        ...
        Utils::InsertImageMemoryBarrier(blitCmd, m_Image, VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_SHADER_READ_BIT,
