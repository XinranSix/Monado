...
namespace Monado {
    ...
    VulkanComputePipeline::VulkanComputePipeline(Ref<Shader> computeShader)
        ...
        Renderer::Submit([instance]() mutable { instance->CreatePipeline(); });
    }
    ...
    void VulkanComputePipeline::CreatePipeline() {
        ...
        VK_CHECK_RESULT(vkCreateComputePipelines(device, m_PipelineCache, 1, &computePipelineCreateInfo, nullptr,
                                                 &m_ComputePipeline));
    }

    void VulkanComputePipeline::Execute(VkDescriptorSet *descriptorSets, uint32_t descriptorSetCount,
        ...
        for (uint32_t i = 0; i < descriptorSetCount; i++) {
            vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_ComputePipelineLayout, 0, 1,
        ...
        // Currently this is here for "safety"
        {
            Timer timer;
            vkWaitForFences(device, 1, &s_ComputeFence, VK_TRUE, UINT64_MAX);
            MND_CORE_TRACE("Compute shader execution took {0} ms", timer.ElapsedMillis());
        }
    }

    void VulkanComputePipeline::Begin() {
        MND_CORE_ASSERT(!m_ActiveComputeCommandBuffer);
    ...
    void VulkanComputePipeline::Dispatch(VkDescriptorSet descriptorSet, uint32_t groupCountX, uint32_t groupCountY,
                                         uint32_t groupCountZ) {
        MND_CORE_ASSERT(m_ActiveComputeCommandBuffer);

        vkCmdBindDescriptorSets(m_ActiveComputeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_ComputePipelineLayout,
    ...
    void VulkanComputePipeline::SetPushConstants(const void *data, uint32_t size) {
        vkCmdPushConstants(m_ActiveComputeCommandBuffer, m_ComputePipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, size,
                           data);
    }
