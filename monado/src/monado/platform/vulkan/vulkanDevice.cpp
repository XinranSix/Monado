...
namespace Monado {
    ...
    VulkanPhysicalDevice::VulkanPhysicalDevice() {
        ...
        if (!selectedPhysicalDevice) {
            MND_CORE_TRACE("Could not find discrete GPU.");
            selectedPhysicalDevice = physicalDevices.back();
        }

        MND_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
        ...
        MND_CORE_ASSERT(queueFamilyCount > 0, "");
        ...
        if (extCount > 0) {
            ...
            if (vkEnumerateDeviceExtensionProperties(m_PhysicalDevice, nullptr, &extCount, &extensions.front()) ==
                VK_SUCCESS) {
                MND_CORE_TRACE("Selected physical device has {0} extensions", extensions.size());
                for (const auto &ext : extensions) {
                    m_SupportedExtensions.emplace(ext.extensionName);
                    MND_CORE_TRACE("  {0}", ext.extensionName);
                }
            }
        }
    ...
    uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const {
        ...
        MND_CORE_ASSERT(false, "Could not find a suitable memory type!");
        return UINT32_MAX;
    }

    Ref<VulkanPhysicalDevice> VulkanPhysicalDevice::Select() { return Ref<VulkanPhysicalDevice>::Create(); }
    ...
    VulkanDevice::VulkanDevice(const Ref<VulkanPhysicalDevice> &physicalDevice,
        ...
        // extension
        MND_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
    ...
    void VulkanDevice::FlushCommandBuffer(VkCommandBuffer commandBuffer) { FlushCommandBuffer(commandBuffer, m_Queue); }

    void VulkanDevice::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue) {
        const uint64_t DEFAULT_FENCE_TIMEOUT = 100000000000;

        MND_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
