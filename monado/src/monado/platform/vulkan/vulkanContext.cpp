...
namespace Monado {
    ...
    static VKAPI_ATTR VkBool32 VKAPI_CALL VulkanDebugReportCallback(VkDebugReportFlagsEXT flags,
        ...
        MND_CORE_WARN("VulkanDebugCallback:\n  Object Type: {0}\n  Message: {1}", objectType, pMessage);
        return VK_FALSE;
    }
    ...
    void VulkanContext::Create() {
        MND_CORE_INFO("VulkanContext::Create");

        MND_CORE_ASSERT(glfwVulkanSupported(), "GLFW must support Vulkan!");
        ...
        appInfo.pApplicationName = "Monado";
        appInfo.pEngineName = "Monado";
...
#define VK_KHR_WIN32_SURFACE_EXTENSION_NAME "VK_KHR_win32_surface"
        ...
        if (s_Validation) {
            ...
            MND_CORE_TRACE("Vulkan Instance Layers:");
            for (const VkLayerProperties &layer : instanceLayerProperties) {
                MND_CORE_TRACE("  {0}", layer.layerName);
            ...
            if (validationLayerPresent) {
                ...
                MND_CORE_ERROR(
                    "Validation layer VK_LAYER_LUNARG_standard_validation not present, validation is disabled");
            }
        }
        ...
        if (s_Validation) {
            ...
            MND_CORE_ASSERT(vkCreateDebugReportCallbackEXT != NULL, "");
            ...
            VK_CHECK_RESULT(
                vkCreateDebugReportCallbackEXT(s_VulkanInstance, &debug_report_ci, nullptr, &m_DebugReportCallback));
        }
        ...
        VK_CHECK_RESULT(
            vkCreatePipelineCache(m_Device->GetVulkanDevice(), &pipelineCacheCreateInfo, nullptr, &m_PipelineCache));
    }

    void VulkanContext::OnResize(uint32_t width, uint32_t height) { m_SwapChain.OnResize(width, height); }

    void VulkanContext::BeginFrame() { m_SwapChain.BeginFrame(); }

    void VulkanContext::SwapBuffers() { m_SwapChain.Present(); }
