...
namespace Monado {
    ...
    RendererCapabilities &VulkanRenderer::GetCapabilities() { return s_Data->RenderCaps; }
    ...
    void VulkanRenderer::RenderMesh(Ref<Pipeline> pipeline, Ref<Mesh> mesh, const glm::mat4 &transform) {
        ...
        for (Submesh &submesh : submeshes) {
            ...
            Renderer::Submit([pipeline, submesh, material, transform]() mutable {
                ...
                std::array<VkDescriptorSet, 2> descriptorSets = { material->GetDescriptorSet().DescriptorSets[0],
                                                                  s_Data->RendererDescriptorSet.DescriptorSets[0] };
                vkCmdBindDescriptorSets(s_Data->ActiveCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0,
                                        descriptorSets.size(), descriptorSets.data(), 0, nullptr);

                glm::mat4 worldTransform = transform * submesh.Transform;

                Buffer uniformStorageBuffer = material->GetUniformStorageBuffer();
                vkCmdPushConstants(s_Data->ActiveCommandBuffer, layout, VK_SHADER_STAGE_VERTEX_BIT, 0,
                                   sizeof(glm::mat4), &worldTransform);
                ...
                vkCmdDrawIndexed(s_Data->ActiveCommandBuffer, submesh.IndexCount, 1, submesh.BaseIndex,
    ...
    void VulkanRenderer::RenderMeshWithoutMaterial(Ref<Pipeline> pipeline, Ref<Mesh> mesh, const glm::mat4 &transform) {
        ...
        for (Submesh &submesh : submeshes) {
            Renderer::Submit([pipeline, submesh, transform]() mutable {
                ...
                // Bind descriptor sets describing shader binding points
                VkDescriptorSet descriptorSet = { vulkanPipeline->GetDescriptorSet() };
    ...
    void VulkanRenderer::RenderQuad(Ref<Pipeline> pipeline, Ref<Material> material, const glm::mat4 &transform) {
        ...
        Renderer::Submit([pipeline, vulkanMaterial, transform]() mutable {
            ...
            vkCmdPushConstants(s_Data->ActiveCommandBuffer, layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4),
                               &transform);
            vkCmdPushConstants(s_Data->ActiveCommandBuffer, layout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(glm::mat4),
                               uniformStorageBuffer.Size, uniformStorageBuffer.Data);
    ...
    void VulkanRenderer::SubmitFullscreenQuad(Ref<Pipeline> pipeline, Ref<Material> material) {
        ...
        Renderer::Submit([pipeline, vulkanMaterial]() mutable {
            ...
            vkCmdPushConstants(s_Data->ActiveCommandBuffer, layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                               uniformStorageBuffer.Size, uniformStorageBuffer.Data);
            vkCmdDrawIndexed(s_Data->ActiveCommandBuffer, s_Data->QuadIndexBuffer->GetCount(), 1, 0, 0, 0);
        });
    }
    ...
    void VulkanRenderer::BeginFrame() {
        Renderer::Submit([]() {
            ...
            MND_CORE_ASSERT(s_Data->ActiveCommandBuffer);
    ...
    void VulkanRenderer::EndRenderPass() {
        // MND_CORE_ASSERT(s_Data->m_ActiveRenderPass, "No active render pass! Have you called Renderer::EndRenderPass
        // twice?");

        Renderer::Submit([]() { vkCmdEndRenderPass(s_Data->ActiveCommandBuffer); });
    }
    ...
    std::pair<Ref<TextureCube>, Ref<TextureCube>> VulkanRenderer::CreateEnvironmentMap(const std::string &filepath) {
        ...
        MND_CORE_ASSERT(envEquirect->GetFormat() == ImageFormat::RGBA32F, "Texture is not HDR!");
        ...
        Ref<VulkanComputePipeline> equirectangularConversionPipeline =
            Ref<VulkanComputePipeline>::Create(equirectangularConversionShader);
        ...
        Renderer::Submit([equirectangularConversionPipeline, envEquirect, envUnfiltered, cubemapSize]() mutable {
            ...
            equirectangularConversionPipeline->Execute(descriptorSet.DescriptorSets.data(),
                                                       descriptorSet.DescriptorSets.size(), cubemapSize / 32,
                                                       cubemapSize / 32, 6);
        ...
        Ref<VulkanComputePipeline> environmentMipFilterPipeline =
            Ref<VulkanComputePipeline>::Create(environmentMipFilterShader);
        ...
        Renderer::Submit([environmentMipFilterPipeline, envUnfiltered, envFiltered, cubemapSize]() mutable {
            ...
            for (uint32_t i = 0; i < 12; i++) {
                ...
                writeDescriptors[i * 2 + 0].dstSet =
                    descriptorSet.DescriptorSets[i]; // Should this be set inside the shader?
                writeDescriptors[i * 2 + 0].pImageInfo = &mipImageInfo;

                Ref<VulkanTextureCube> envUnfilteredCubemap = envUnfiltered.As<VulkanTextureCube>();
                writeDescriptors[i * 2 + 1] = *shader->GetDescriptorSet("inputTexture");
                writeDescriptors[i * 2 + 1].dstSet =
                    descriptorSet.DescriptorSets[i]; // Should this be set inside the shader?
        ...
        Ref<VulkanComputePipeline> environmentIrradiancePipeline =
            Ref<VulkanComputePipeline>::Create(environmentIrradianceShader);
        Ref<TextureCube> irradianceMap =
            TextureCube::Create(ImageFormat::RGBA32F, irradianceMapSize, irradianceMapSize);
        ...
        Renderer::Submit([environmentIrradiancePipeline, irradianceMap, envFiltered]() mutable {
            ...
            vkUpdateDescriptorSets(device, writeDescriptors.size(), writeDescriptors.data(), 0, NULL);
            environmentIrradiancePipeline->Execute(descriptorSet.DescriptorSets.data(),
                                                   descriptorSet.DescriptorSets.size(), irradianceMap->GetWidth() / 32,
