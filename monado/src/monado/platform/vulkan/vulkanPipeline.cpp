...
namespace Monado {
    ...
    void VulkanPipeline::Invalidate() {
        ...
        Renderer::Submit([instance]() mutable {
            ...
            MND_CORE_ASSERT(instance->m_Specification.Shader);
            Ref<VulkanShader> vulkanShader = Ref<VulkanShader>(instance->m_Specification.Shader);
            Ref<VulkanFramebuffer> framebuffer =
                instance->m_Specification.RenderPass->GetSpecification().TargetFramebuffer.As<VulkanFramebuffer>();
            ...
            // descriptor set layout In a more complex scenario you would have different pipeline layouts for different
            ...
            // state machine A pipeline is then stored and hashed on the GPU making pipeline changes very fast Note:
            // There are still a few dynamic states that are not directly part of the pipeline (but the info that they
            // are used is)
            ...
            // Most states are baked into the pipeline, but there are still a few dynamic states that can be changed
            // within a command buffer To be able to change these we need do specify which dynamic states will be
            // changed using this pipeline. Their actual states are set later on in the command buffer. For this example
            ...
            // This example does not make use fo multi sampling (for anti-aliasing), the state must still be set and
            ...
            MND_CORE_WARN("VulkanPipeline - Updating {0} descriptor sets", writeDescriptors.size());
