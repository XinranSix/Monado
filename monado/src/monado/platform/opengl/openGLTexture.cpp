...
namespace Monado {
    ...
    OpenGLTexture2D::OpenGLTexture2D(TextureFormat format, unsigned int width, unsigned int height, TextureWrap wrap)
        ...
        MND_RENDER_1(self, {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, MonadoToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height,
                         0, MonadoToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, nullptr);
    ...
    OpenGLTexture2D::OpenGLTexture2D(const std::string &path, bool srgb) : m_FilePath(path) {
        ...
        MND_RENDER_S1(srgb, {
            ...
            if (srgb) {
                ...
                MND_CORE_INFO("Creating srgb texture width {0} mips", levels);
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, MonadoToOpenGLTextureFormat(self->m_Format), self->m_Width,
                             self->m_Height, 0, srgb ? GL_SRGB8 : MonadoToOpenGLTextureFormat(self->m_Format),
                             GL_UNSIGNED_BYTE, self->m_ImageData.Data);
                glGenerateMipmap(GL_TEXTURE_2D);
    ...
    OpenGLTexture2D::~OpenGLTexture2D() {
        MND_RENDER_S({ glDeleteTextures(1, &self->m_RendererID); });
    }

    void OpenGLTexture2D::Bind(uint32_t slot) const {
        MND_RENDER_S1(slot, { glBindTextureUnit(slot, self->m_RendererID); });
    }
    ...
    void OpenGLTexture2D::Unlock() {
        ...
        MND_RENDER_S({
            glTextureSubImage2D(self->m_RendererID, 0, 0, 0, self->m_Width, self->m_Height,
                                MonadoToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
        });
    }
    ...
    OpenGLTextureCube::OpenGLTextureCube(const std::string &path) : m_FilePath(path) {
        ...
        MND_RENDER_S3(faces, faceWidth, faceHeight, {
            glGenTextures(1, &self->m_RendererID);
            ...
            auto format = MonadoToOpenGLTextureFormat(self->m_Format);
