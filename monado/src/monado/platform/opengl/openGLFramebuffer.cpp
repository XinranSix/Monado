...
namespace Monado {
    ...
    namespace Utils {
        ...
        static GLenum DataType(GLenum format) {
           ...
           MND_CORE_ASSERT(false, "Unknown format!");
            return 0;
        }

        static void AttachColorTexture(RendererID id, int samples, GLenum format, uint32_t width, uint32_t height,
                                       int index) {
    ...
    OpenGLFramebuffer::~OpenGLFramebuffer() {
        ...
        Renderer::Submit([instance]() { glDeleteFramebuffers(1, &instance->m_RendererID); });
    }
    ...
    void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height, bool forceRecreate) {
        ...
        Renderer::Submit([instance]() mutable {
            ...
            if (instance->m_ColorAttachmentFormats.size()) {
                ...
                Utils::CreateTextures(multisample, instance->m_ColorAttachments.data(),
                                      instance->m_ColorAttachments.size());
                ...
                for (int i = 0; i < instance->m_ColorAttachments.size(); i++) {
                    ...
                    case FramebufferTextureFormat::RGBA8:
                        Utils::AttachColorTexture(instance->m_ColorAttachments[i], instance->m_Specification.Samples,
            ...
            if (instance->m_DepthAttachmentFormat != FramebufferTextureFormat::None) {
                ...
                case FramebufferTextureFormat::DEPTH24STENCIL8:
                    Utils::AttachDepthTexture(instance->m_DepthAttachment, instance->m_Specification.Samples,
                                              GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL_ATTACHMENT, instance->m_Width,
                                              instance->m_Height);
                    break;
                case FramebufferTextureFormat::DEPTH32F:
                    Utils::AttachDepthTexture(instance->m_DepthAttachment, instance->m_Specification.Samples,
                                              GL_DEPTH_COMPONENT32F, GL_DEPTH_ATTACHMENT, instance->m_Width,
                                              instance->m_Height);
                    break;
                }
            }
            ...
            if (instance->m_ColorAttachments.size() > 1) {
               MND_CORE_ASSERT(instance->m_ColorAttachments.size() <= 4);
			...
			if (multisample)
			{
				...
				// TODO: Create Monado texture object based on format here
