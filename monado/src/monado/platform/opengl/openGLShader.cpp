...
#if UNIFORM_LOGGING
    #define MND_LOG_UNIFORM(...) MND_CORE_WARN(__VA_ARGS__)
#else
    #define MND_LOG_UNIFORM
#endif
...
#define PRINT_SHADERS 1
    ...
    OpenGLShader::OpenGLShader(const std::string &filepath, bool forceRecompile) : m_AssetPath(filepath) {
    ...
    void OpenGLShader::ClearUniformBuffers() { s_UniformBuffers.clear(); }
    ...
    void OpenGLShader::CompileOrGetVulkanBinary(std::unordered_map<uint32_t, std::vector<uint32_t>> &outputBinary,
                                                bool forceCompile) {
            ...
            if (outputBinary[stage].size() == 0) {
                ...
                // Compile shader
                {
                    ...
                    shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(
                        shaderSource, GLShaderStageToShaderC(stage), m_AssetPath.c_str(), options);
    ...
    void
    OpenGLShader::CompileOrGetOpenGLBinary(const std::unordered_map<uint32_t, std::vector<uint32_t>> &vulkanBinaries,
                                           bool forceCompile) {
        if (m_RendererID)
...

            {
                ...
                if (!shaderStageData.size()) {
                    ...
                    shaderc::SpvCompilationResult module =
                        compiler.CompileGlslToSpv(source, GLShaderStageToShaderC(stage), m_AssetPath.c_str(), options);
...

                    {
                        std::filesystem::path p = m_AssetPath;
                        auto path =
                            cacheDirectory / (p.filename().string() + GLShaderStageCachedOpenGLFileExtension(stage));
                        std::string cachedFilePath = path.string();
                        FILE *f = fopen(cachedFilePath.c_str(), "wb");
        ...
        if (isLinked == GL_FALSE) {
            ...
            MND_CORE_ERROR("Shader compilation failed ({0}):\n{1}", m_AssetPath, &infoLog[0]);
        ...
        for (auto &[bufferName, buffer] : m_Buffers) {
            for (auto &[name, uniform] : buffer.Uniforms) {
                ...
                if (location == -1)
                    MND_CORE_WARN("{0}: could not find uniform location {0}", name);
        ...
        for (const spirv_cross::Resource &resource : res.push_constant_buffers) {
            ...
            for (int i = 0; i < memberCount; i++) {
                ...
                buffer.Uniforms[uniformName] =
                    ShaderUniform(uniformName, SPIRTypeToShaderUniformType(type), size, offset);
            }
    ...
    void OpenGLShader::Reflect(std::vector<uint32_t> &data) {
        ...
        MND_CORE_TRACE("OpenGLShader::Reflect - {0}", m_AssetPath);
        MND_CORE_TRACE("   {0} Uniform Buffers", res.uniform_buffers.size());
        MND_CORE_TRACE("   {0} Resources", res.sampled_images.size());
        ...
        for (const spirv_cross::Resource &resource : res.uniform_buffers) {
            ...
            if (s_UniformBuffers.find(bindingPoint) == s_UniformBuffers.end()) {
                ...
                MND_CORE_TRACE("Created Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes",
                               buffer.BindingPoint, buffer.Name, buffer.Size);

                glBindBuffer(GL_UNIFORM_BUFFER, 0);
            } else {
                // Validation
                ShaderUniformBuffer &buffer = s_UniformBuffers.at(bindingPoint);
                MND_CORE_ASSERT(buffer.Name == resource.name); // Must be the same buffer
                if (bufferSize > buffer.Size)                  // Resize buffer if needed
                {
                    ...
                    MND_CORE_TRACE("Resized Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes",
                                   buffer.BindingPoint, buffer.Name, buffer.Size);
                }
            }
        }
    ...
    int32_t OpenGLShader::GetUniformLocation(const std::string &name) const {
        ...
        if (result == -1)
            MND_CORE_WARN("Could not find uniform '{0}' in shader", name);
    ...
    size_t OpenGLShader::GetHash() const { return std::hash<std::string> {}(m_AssetPath); }
    ...
    void OpenGLShader::SetUniformBuffer(const std::string &name, const void *data, uint32_t size) {
        ...
        MND_CORE_ASSERT(uniformBuffer);
        MND_CORE_ASSERT(uniformBuffer->Size >= size);
    ...
    void OpenGLShader::SetUniform(const std::string &fullname, float value) {
        Renderer::Submit([=]() {
            MND_CORE_ASSERT(m_UniformLocations.find(fullname) != m_UniformLocations.end());
    ...
    void OpenGLShader::SetUInt(const std::string &name, uint32_t value) {
        Renderer::Submit([=]() { UploadUniformUInt(name, value); });
    }
    ...
    void OpenGLShader::UploadUniformFloat(uint32_t location, float value) { glUniform1f(location, value); }
    ...
    void OpenGLShader::UploadUniformFloat(const std::string &name, float value) {
        ...
        if (location != -1)
            ...
            MND_LOG_UNIFORM("Uniform '{0}' not found!", name);
    }
