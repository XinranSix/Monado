...
namespace Monado {
    ...
    RendererCapabilities &OpenGLRenderer::GetCapabilities() { return s_Data->RenderCaps; }
    ...
    void OpenGLRenderer::BeginRenderPass(const Ref<RenderPass> &renderPass) {
        ...
        if (clear) {
            const glm::vec4 &clearColor =
                renderPass->GetSpecification().TargetFramebuffer->GetSpecification().ClearColor;
            Renderer::Submit([=]() { Utils::Clear(clearColor.r, clearColor.g, clearColor.b, clearColor.a); });
        }
    }
    ...
    void OpenGLRenderer::SubmitFullscreenQuad(Ref<Pipeline> pipeline, Ref<Material> material) {
        ...
        Renderer::Submit([depthTest]() { Utils::DrawIndexed(6, PrimitiveType::Triangles, depthTest); });
    }
    ...
    std::pair<Ref<TextureCube>, Ref<TextureCube>> OpenGLRenderer::CreateEnvironmentMap(const std::string &filepath) {
        ...
        Ref<OpenGLTextureCube> envUnfiltered =
            TextureCube::Create(ImageFormat::RGBA32F, cubemapSize, cubemapSize).As<OpenGLTextureCube>();
        ...
        Ref<OpenGLTextureCube> envFiltered =
            TextureCube::Create(ImageFormat::RGBA32F, cubemapSize, cubemapSize).As<OpenGLTextureCube>();
        ...
        Renderer::Submit([envFilteringShader, envUnfiltered, envFiltered, cubemapSize]() {
                 ...
                 level++, size /= 2) // <= ?
            {
                ...
                GLint roughnessUniformLocation =
                    glGetUniformLocation(envFilteringShader->GetRendererID(), "u_Uniforms.Roughness");
                MND_CORE_ASSERT(roughnessUniformLocation != -1);
        ...
        Ref<OpenGLTextureCube> irradianceMap =
            TextureCube::Create(ImageFormat::RGBA32F, irradianceMapSize, irradianceMapSize).As<OpenGLTextureCube>();
