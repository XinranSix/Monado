...
namespace Monado {
    ...
    Ref<Texture2D> OpenGLMaterial::GetTexture2D(const std::string &name) {
        ...
        uint32_t slot = decl->GetRegister();
        MND_CORE_ASSERT(slot < m_Texture2Ds.size(), "Texture slot is invalid");
    ...
    void OpenGLMaterial::UpdateForRendering() {
        ...
        Renderer::Submit([shader]() { glUseProgram(shader->GetRendererID()); });
        ...
        for (size_t i = 0; i < m_Textures.size(); i++) {
            ...
            if (texture) {
                Renderer::Submit([i, texture]() {
                    MND_CORE_ASSERT(texture->GetType() == TextureType::TextureCube);
