...
public:
    ...
    virtual void OnAttach() override {
        ...
        m_Mesh.reset(new Monado::Mesh("monadoassets/models/m1911/m1911.fbx"));
        m_MeshMaterial.reset(new Monado::MaterialInstance(m_Mesh->GetMaterial()));

        m_QuadShader = Monado::Shader::Create("monadoassets/shaders/quad.glsl");
        m_HDRShader = Monado::Shader::Create("monadoassets/shaders/hdr.glsl");

        m_SphereMesh.reset(new Monado::Mesh("monadoassets/models/Sphere1m.fbx"));
        m_PlaneMesh.reset(new Monado::Mesh("monadoassets/models/Plane1m.obj"));

        m_GridShader = Monado::Shader::Create("monadoassets/shaders/Grid.glsl");
        ...
        // Editor
        m_CheckerboardTex.reset(Monado::Texture2D::Create("sandbox/assets/editor/Checkerboard.tga"));
        ...
        m_Framebuffer.reset(Monado::Framebuffer::Create(1280, 720, Monado::FramebufferFormat::RGBA16F));
        m_FinalPresentBuffer.reset(Monado::Framebuffer::Create(1280, 720, Monado::FramebufferFormat::RGBA8));

        float x = -4.0f;
        float roughness = 0.0f;
        for (int i = 0; i < 8; i++) {
            Monado::Ref<Monado::MaterialInstance> mi(new Monado::MaterialInstance(m_SphereMesh->GetMaterial()));
        ...
        m_VertexBuffer.reset(Monado::VertexBuffer::Create());
        m_VertexBuffer->SetData(data, 4 * sizeof(QuadVertex));

        uint32_t *indices = new uint32_t[6] {
            0, 1, 2, 2, 3, 0,
        };
        m_IndexBuffer.reset(Monado::IndexBuffer::Create());
    ...
    virtual void OnUpdate(Monado::TimeStep ts) override {
        // THINGS TO LOOK AT:
        // - BRDF LUT
    ...
    virtual void OnImGuiRender() override {
        ...
        ImGui::Separator();
        {
            ...
            if (ImGui::Button("...##Mesh")) {
                std::string filename = Monado::Application::Get().OpenFile("");
                if (filename != "") {
            ...
            if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (ImGui::IsItemHovered()) {
                    ...
                    if (ImGui::IsItemClicked()) {
                        std::string filename = Monado::Application::Get().OpenFile("");
                        if (filename != "")
                            m_AlbedoInput.TextureMap.reset(Monado::Texture2D::Create(filename, m_AlbedoInput.SRGB));
                    }
                }
                ImGui::SameLine();
                ImGui::BeginGroup();
                ...
                if (ImGui::Checkbox("sRGB##AlbedoMap", &m_AlbedoInput.SRGB)) {
                    if (m_AlbedoInput.TextureMap)
                        m_AlbedoInput.TextureMap.reset(
                            Monado::Texture2D::Create(m_AlbedoInput.TextureMap->GetPath(), m_AlbedoInput.SRGB));
                }
                ImGui::EndGroup();
                ImGui::SameLine();
            ...
            if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (ImGui::IsItemHovered()) {
                    ...
                    if (ImGui::IsItemClicked()) {
                        std::string filename = Monado::Application::Get().OpenFile("");
                        if (filename != "")
                            m_NormalInput.TextureMap.reset(Monado::Texture2D::Create(filename));
                    }
                }
                ImGui::SameLine();
            ...
            if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (ImGui::IsItemHovered()) {
                    ...
                    if (ImGui::IsItemClicked()) {
                        std::string filename = Monado::Application::Get().OpenFile("");
                        if (filename != "")
                            m_MetalnessInput.TextureMap.reset(Monado::Texture2D::Create(filename));
                    }
                }
                ImGui::SameLine();
            ...
            if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (ImGui::IsItemHovered()) {
                    ...
                    if (ImGui::IsItemClicked()) {
                        std::string filename = Monado::Application::Get().OpenFile("");
                        if (filename != "")
                            m_RoughnessInput.TextureMap.reset(Monado::Texture2D::Create(filename));
                    }
                }
                ImGui::SameLine();
        ...
        auto [wx, wy] = Monado::Application::Get().GetWindow().GetWindowPos();
        posX -= wx;
        posY -= wy;
    ...
    virtual void OnEvent(Monado::Event &event) override {}

private:
    Monado::Ref<Monado::Shader> m_QuadShader;
    Monado::Ref<Monado::Shader> m_HDRShader;
    Monado::Ref<Monado::Shader> m_GridShader;
    Monado::Ref<Monado::Mesh> m_Mesh;
    Monado::Ref<Monado::Mesh> m_SphereMesh, m_PlaneMesh;
    Monado::Ref<Monado::Texture2D> m_BRDFLUT;

    Monado::Ref<Monado::MaterialInstance> m_MeshMaterial;
    Monado::Ref<Monado::MaterialInstance> m_GridMaterial;
    std::vector<Monado::Ref<Monado::MaterialInstance>> m_MetalSphereMaterialInstances;
    std::vector<Monado::Ref<Monado::MaterialInstance>> m_DielectricSphereMaterialInstances;
    ...
    struct AlbedoInput {
        ...
        Monado::Ref<Monado::Texture2D> TextureMap;
        bool SRGB = true;
    ...
    std::unique_ptr<Monado::Framebuffer> m_Framebuffer, m_FinalPresentBuffer;

    Monado::Ref<Monado::VertexBuffer> m_VertexBuffer;
    Monado::Ref<Monado::IndexBuffer> m_IndexBuffer;
    Monado::Ref<Monado::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;
    ...
    // Editor resources
    Monado::Ref<Monado::Texture2D> m_CheckerboardTex;
};

class Sandbox : public Monado::Application {
public:
    Sandbox() { MND_TRACE("Hello!"); }

    virtual void OnInit() override { PushLayer(new EditorLayer()); }
};

Monado::Application *Monado::CreateApplication() { return new Sandbox(); }
