...
public:
    ...
    virtual void OnAttach() override {
        ...
        m_Mesh.reset(new Monado::Mesh("sandbox/assets/meshes/cerberus.fbx"));
        m_SphereMesh.reset(new Monado::Mesh("sandbox/assets/models/Sphere.fbx"));
    ...
    virtual void OnUpdate() override {
        ...
        Monado::UniformBufferDeclaration<sizeof(mat4) * 2 + sizeof(vec3) * 4 + sizeof(float) * 8, 14> simplePbrShaderUB;
...
private:
    std::unique_ptr<Monado::Shader> m_Shader;
    std::unique_ptr<Monado::Shader> m_PBRShader;
    std::unique_ptr<Monado::Shader> m_SimplePBRShader;
    std::unique_ptr<Monado::Shader> m_QuadShader;
    std::unique_ptr<Monado::Shader> m_HDRShader;
    std::unique_ptr<Monado::Mesh> m_Mesh;
    std::unique_ptr<Monado::Mesh> m_SphereMesh;
    std::unique_ptr<Monado::Texture2D> m_BRDFLUT;

    struct AlbedoInput {
        glm::vec3 Color = {
            0.972f, 0.96f, 0.915f
        }; // Silver, from https://docs.unrealengine.com/en-us/Engine/Rendering/Materials/PhysicallyBased
        std::unique_ptr<Monado::Texture2D> TextureMap;
        bool SRGB = true;
    ...
    std::unique_ptr<Monado::VertexBuffer> m_VertexBuffer;
    std::unique_ptr<Monado::IndexBuffer> m_IndexBuffer;
    ...
    // Editor resources
    std::unique_ptr<Monado::Texture2D> m_CheckerboardTex;
};
